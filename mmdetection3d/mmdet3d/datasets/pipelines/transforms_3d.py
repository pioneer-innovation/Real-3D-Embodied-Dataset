...
class Open3dShow(object):
    ...
    def AnnoToPc(self,coors):
        coors = coors[:, [0, 2, 1]]
        coors[:, 2] = -coors[:, 2]
        coors[:, 1] = coors[:, 1] - 1.1
        return coors

    def DrawBox(self,corners):
        box_list = []
        # line_set = np.array([[0, 1], [1, 3], [0, 2], [2, 3],
        #                      [4, 5], [4, 6], [5, 7], [6, 7],
        #                      [0, 4], [1, 5], [2, 6], [3, 7]])
        line_set = np.array([[0,1],[2,3],[4,5],[6,7],
                             [1,2],[1,5],[2,6],[5,6],
                             [0,4],[0,3],[4,7],[3,7]])
    ...
    def __call__(self, input_dict):
        ...
        Returns:
            dict: Results after filtering, 'points' keys are updated \
                in the result dict.
        """
        if len(input_dict['ann_info']['gt_bboxes_3d'])!=0:
            print(input_dict['pts_filename'])
            points = input_dict['points']
            gt_boxes = input_dict['ann_info']['gt_bboxes_3d'].corners.numpy()

            xyz = points.tensor[:,0:3].numpy()
            xyz = self.AnnoToPc(xyz)
            color = points.tensor[:,3:6].numpy()
            pc = o3d.geometry.PointCloud()
            pc.points = o3d.utility.Vector3dVector(xyz)
            pc.colors = o3d.utility.Vector3dVector(color)

            for i in range(gt_boxes.shape[0]):
                gt_boxes[i,:] = self.AnnoToPc(gt_boxes[i,:])

            boxes = self.DrawBox(gt_boxes)
            boxes.append(pc)
