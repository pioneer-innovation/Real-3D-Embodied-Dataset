...
class R3ADData(object):
    ...
    def __init__(self, root_path, split='train', use_v1=False):
        ...
        self.split = split
        self.split_dir = osp.join(root_path, 'active_R3AD_trainval')
        self.classes = [
        ...
        split_file = osp.join(self.split_dir, f'active_{split}_data_idx.txt')
        mmcv.check_file_exist(split_file)
        self.sample_id_list = mmcv.list_from_file(split_file)

        self.cloud_path_list = [line.rstrip() for line in open(self.split_dir + '/' + 'active_cloud_path.txt')]
        self.anno_path_list = [line.rstrip() for line in open(self.split_dir + '/' + 'active_anno_path.txt')]
